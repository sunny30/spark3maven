...
object JsonFlatten {
  ...
  implicit class DF(df:DataFrame){
    ...
    implicit def flattenDF(df1:DataFrame=df):DataFrame={
      val fields = df1.schema.fields
      val len = df1.schema.length
      val fieldNames = df1.schema.fields.map(x=>x.name)
      ...
      for(i<-0 to len-1){
        ...
        cfield.dataType match {
          ...
          case arrayType: ArrayType =>{
            val columns = fieldNames.filter(_!=cfield.name).map(x=>expr(x))++Seq(explode(col(cfield.name)).as(cfield.name))
            val flatdf = df1.select(columns:_*)
            return flattenDF(flatdf)
